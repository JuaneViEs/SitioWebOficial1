...
function generarConstancia() {
    ...
    doc.setFontSize(18);
    const tituloY = 40; // Ajustamos el título para que esté más abajo
...
Constancia  que  se  expide  a  petición  de  parte  interesada  en  el Municipio Torres,Parroquia     Trinidad  Samuel,  Estado  Lara  a  ${fechaTexto}.
    ...
    // --- Justificar el texto de manera normal ---
    doc.setFontSize(12);
    const lines = doc.splitTextToSize(texto, docWidth - 2 * margin);

    // Se coloca el texto en el documento sin "forzar" la justificación de manera extrema
    let yOffset = tituloY + 10; // Comienza después del título y el subrayado

    lines.forEach(line => {
        // Justificar el texto de manera normal, sin separaciones extremas
        doc.text(line, margin, yOffset, { maxWidth: docWidth - 2 * margin, align: 'justify' });
        yOffset += 6; // Espaciado entre líneas (puedes ajustarlo según lo que prefieras)
    });
